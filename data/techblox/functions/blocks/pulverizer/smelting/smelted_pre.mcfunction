...
execute store result score @s tb.pulv.items2 run data get block ~ ~ ~ Items[{Slot: 2b}].Count 1
# Increase Output Items
scoreboard players operation @s tb.pulv.items2 += @s tb.pulv.itemInc
#scoreboard players add @s tb.pulv.items2 1
# Set current Input Items value
execute store result score @s tb.pulv.items0 run data get block ~ ~ ~ Items[{Slot: 0b}].Count 1
# Decrease Input Items
scoreboard players remove @s tb.pulv.items0 1